...
public class AppTimeUtils {
	...
	public static String formatDate(Date jsonDate) {
		return formatDate(jsonDate, "yyyy-MM-dd HH:mm:ss");
	}
	...
	public static String getInitTimeString(Context context, String jsonTime) {
		...
		if (jsonTime != null) {
			...
			if (yearGap == 0 && monthGap == 0 && dateGap == 0) {
				SimpleDateFormat format = new SimpleDateFormat(context.getString(R.string.talk_today_format));
				str = format.format(jsonDate);
			} else if (yearGap == 0 && monthGap == 0 && dateGap == 1) {
				SimpleDateFormat format = new SimpleDateFormat(context.getString(R.string.talk_yesterday_format));
				str = format.format(jsonDate);
			} else if (yearGap == 0 && monthGap == 0 && dateGap < 5) {
				Calendar cal = Calendar.getInstance();
				cal.setTime(jsonDate);
				SimpleDateFormat format = new SimpleDateFormat(context.getString(R.string.talk_today_format));
				str = getWeekText(context, cal.get(Calendar.DAY_OF_WEEK)) + " " + format.format(jsonDate);
			} else {
				SimpleDateFormat format = new SimpleDateFormat(context.getString(R.string.talk_date_format));
				str = format.format(jsonDate);
			}
			return str;
		}

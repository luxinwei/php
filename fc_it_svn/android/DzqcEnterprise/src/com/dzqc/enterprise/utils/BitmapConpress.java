...
public class BitmapConpress {
	...
	public static Bitmap getimage(String srcPath, float height, float width, int size) {
		...
		if (w > width || h > height) {
			...
			while ((h / be) < DensityUtils.dp2Px(DzqcEnterprise.getInstance(), 50) && be > 0) {
				be--;
			}
		}
		if (be <= 0)
		...
		if (bitmap != null) {
			...
			return BitmapFactory.decodeResource(DzqcEnterprise.getInstance().getResources(), R.drawable.error_img);
	}
	...
	public static Bitmap fitTheScreenSizeImage(Bitmap m, float width, float height) {
		if (m != null) {
			...
			float h = height / hh;
			csale = w > h ? h : w;
			while (hh * csale < DensityUtils.dp2Px(DzqcEnterprise.getInstance(), 50)) {
				csale += 0.1f;
			}

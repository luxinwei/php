	...
	public void afterTextChanged(Editable s) {
		...
		if (s.length() > 0) {
			...
			if (count==listEdit.size()) {
				...
				tvRegister.setBackgroundDrawable(DzqcEnterprise.getInstance().getResources().getDrawable(R.drawable.user_login_btn));
			}
		} else {
			count=0;
			tvRegister.setEnabled(false);
			tvRegister.setBackgroundDrawable(DzqcEnterprise.getInstance().getResources().getDrawable(R.drawable.btn_disable_style));
		}
	}

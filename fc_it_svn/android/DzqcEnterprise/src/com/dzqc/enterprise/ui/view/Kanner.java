package com.dzqc.enterprise.ui.view;
...
public class Kanner extends FrameLayout {
	...
	private void initLayout() {
		imageViews.clear();
		View view = LayoutInflater.from(context).inflate(R.layout.view_kanner, this, true);
	...
	private void initImgFromRes(int[] imagesRes) {
		...
		for (int i = 0; i < count; i++) {
			...
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(DensityUtils.dp2Px(context, 10), DensityUtils.dp2Px(context, 10));
			params.leftMargin = 5;
	...
	private void initImgFromNet(String[] imagesUrl) {
		...
		for (int i = 0; i <= count + 1; i++) {
			...
			iv.setBackgroundResource(R.drawable.loading);
			if (i == 0)
				HttpImage.loadImage(iv, imagesUrl[count - 1]);
			else if (i == count + 1)
				HttpImage.loadImage(iv, imagesUrl[0]);
			else
				HttpImage.loadImage(iv, imagesUrl[i - 1]);
			imageViews.add(iv);
		}
	}

...
jQuery(document).ready(function(){
	...
	jQuery('.changetheme a').click(function(){
		...
		if(jQuery('#addonstyle').length == 0) {
			if(c != 'default') {
				jQuery('head').append('<link id="addonstyle" rel="stylesheet" href="'+_PUBLIC_+'admin/css/style.'+c+'.css" type="text/css" />');
				jQuery.cookie("addonstyle", c, { path: '/' });
			}
		} else {
			if(c != 'default') {
				jQuery('#addonstyle').attr('href',_PUBLIC_+'admin/css/style.'+c+'.css');
				jQuery.cookie("addonstyle", c, { path: '/' });
			} else {

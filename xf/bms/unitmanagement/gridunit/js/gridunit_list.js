    ...
    if(success){
   	 ...
   	 for(j=0;j<data.length;j++){ 
   		...
   		content+="</td>";
			...
			content+="<td>"+data[j]["scopeCoordinates"]+"</td>";                      
			content+="<td>"+ClearTrim(data[j]["parentDepName"])+"</td>";                      
 
			content+="<td class=\"tc\">";   
			content+="<a class=\"hmui-btn-primary\" href=\"gridunit_detail.php?id="+id+"\"></a>";
			content+="</td>";
			content+="</tr>"; 
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="gridunit_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
		var tourl="gridunit_xls.php?ids="+ids;
		window.open(tourl);
});

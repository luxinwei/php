	...
	// $("#pageNav").parent().after(JSON.stringify(resultobj));	   
     ...
     if(success){
         ...
         for(j=0;j<data.length;j++){
             ...
             content+="<td>"+data[j]["liaisonOfficer"]+"</td>";
             content+="<td>"+data[j]["liaisonOfficerPhone"]+"</td>";
             content+="<td>"+fnChangeName(data[j]["fireServiceCode"],fireServiceCode_jsarry)+"</td>"; //中心级别
              content+="<td class=\"tc\">";
             content+="<a class=\"hmui-btn-primary\" href=\"firedepartment_detail.php?id="+id+"\"></a>";
             content+="</td>";
             content+="</tr>";
...
$("#btn_add").bind("click", function(){
    var tourl="firedepartment_add.php";
...
$("#btn_edit").bind("click", function(){
    ...
    var key=row.value;
    window.location.href="firedepartment_update.php?id="+key;
});
//删除动作---删除单条数据=========================================
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
    var tourl="firedepartment_xls.php?"+params;
    window.open(tourl);
});

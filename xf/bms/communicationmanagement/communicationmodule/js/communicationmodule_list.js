...
function search(){
	...
	if(params!="")params+="&";
	params+="uri=communication_modules";
	  ...
	  var success=resultobj.success; 
	     ...
	     if(success){
	    	 ...
	    	 for(j=0;j<data.length;j++){ 
				...
				content+="</td>";                                                                
				content+="<td>"+data[j]["sid"]+"</td>";  
				content+="<td>"+data[j]["depName"]+"</td>";    
				content+="<td>"+data[j]["name"]+"</td>";
 				content+="<td>"+fnChangeName(data[j]["comMouldCode"],comMouldCode_jsarry)+"</td>"; //中心级别
				content+="<td>"+data[j]["position"]+"</td>";             
				content+="<td>"+data[j]["description"]+"</td>";   
				content+="<td class=\"tc\">";   
				content+="<a class=\"hmui-btn-primary\" href=\"communicationmodule_detail.php?id="+id+"\"></a>";
				content+="</td>"; 
				content+="</tr>"; 
...
$("#btn_add").bind("click", function(){ 
	var tourl="communicationmodule_add.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="communicationmodule_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================

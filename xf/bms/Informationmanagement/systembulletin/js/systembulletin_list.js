$("#btn_build").bind("click", function(){window.location.href="systembulletin_build.php";});
$("#btn_prc").bind("click", function(){
	var tourl="systembulletin_pic.php";
	var iWidth=1000; 
	var iHeight=800; 
...
function processData(resultobj){
	...
	if(success){
		...
		for(j=0;j<data.length;j++){
			...
			content+="<td>"+fnChangeName(data[j]["bulletinAreaCode"],bulletinAreaCode_jsarry)+"</td>";
			content+="<td>"+ClearTrim(data[j]["createUser"])+"</td>";
			content+="<td>"+ClearTrim(timeFormat(data[j]["createTime"]))+"</td>";
			content+="<td>"+fnChangeName(data[j]["dataStateCode"],dataStateCode_jsarry)+"</td>";
			content+="<td class=\"tc\">";
			content+="<a class=\"hmui-btn-primary\" href=\"systembulletin_detail.php?id="+id+"\"></a>";
			content+="</td>";
			content+="</tr>";
...
var tourl="management_build.php";  
...
var key=row.value;
window.location.href="management_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
  ...
  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
	...
	var url=rootPath+"/com/base/InterfaceDeleteAction.php";
	 ...
	 if(row.row>1){
	    	alert(row.value);
	    	 params={uri:"system_notices/batch",commitData:row.value};

...
class DicationaryUtil {
	...
	public function getLibRadioBox($name,$itemno,$libsArray){
		$content="";
		foreach ($libsArray as $key => $value) {
			$selected="";
			if(strcmp($itemno,$key)==0)$selected="checked=\"checked\"";
			$content.="<input name=\"".$name."\"  type=\"radio\" ".$selected."  value=\"".$key."\" />".trim($value);
	...
	public function getCodeCheckBox($codeno,$name,$valueList,$excludeList){
		...
		foreach ($key_array as $mapkey){
			...
			$content.="<span class=\"checkbox_name\"><input name=\"".$name."\"  type=\"checkbox\" ".$checkstr."  value=\"".$itemno."\" \\/>".trim($itemname)."</span>";	
		}
		return $content;
	}
	//字典表单选框
	public function getCodeRadioBox($codeno,$name,$value,$excludeList){
		...
		foreach ($key_array as $mapkey){
			...
			$content.="<span class=\"rediobox_name\"><input name=\"".$name."\"  type=\"radio\" ".$checkstr."  value=\"".$itemno."\" />".trim($itemname)."</span>";
	...
	private function getCodeArray(){
		$baseDataFimcode=array();
		$list1=DB::getInstance()->execQuery("select * from dicationary_types");
		foreach ($list1 as $obj1){
			$code2Array=array();
			$list2=DB::getInstance()->execQuery("select * from dicationary_datas where typeid='".$obj1["ID"]."' order by SORT asc");
			foreach ($list2 as $obj2){
				$code2Array[$obj2["VALUE"]]=$obj2["TITLE"];
			}
			
			$baseDataFimcode[$obj1["CODE"]]=array("CODENO"=> $obj1[CODE],"TITLE"=> $obj1["TITLE"],"LIBS"=>$code2Array);
		}
		
	
		return $baseDataFimcode;
	}
}
?>
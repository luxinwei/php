...
function initcode1($loginName,$pwd,$appCode){
	...
	if($success){
		$sql_array=array();
		$sql_array[]="delete from dictionary_types";
		...
		foreach ($data as $obj){
			...
			$sql_array[]=DB::getInstance()->getInsertSql("dictionary_types", $colobj);
		}
...
function initcode2($loginName,$pwd,$appCode){
	...
	if($success){
		$sql_array=array();
		$sql_array[]="delete from dictionary_datas";
		$data=$resutltArray->data;
		foreach ($data as $obj){
			$colobj=array();
			$colobj["ID"]=$obj->id;
			...
			$sql_array[]=DB::getInstance()->getInsertSql("dictionary_datas", $colobj);
		}
...
function getFunData($uri,$loginName,$pwd,$appCode){
	...
	$ursl.="&params=".urlencode ("{\"pageSize\":\"".$pageSize."\",\"currentPage\":\"".$currentPage."\"}");

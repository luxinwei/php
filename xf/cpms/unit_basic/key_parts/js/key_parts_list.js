	...
	////$("#pageNav").parent().after(JSON.stringify(resultobj));
     ...
     if(success){
    	 ...
    	 for(j=0;j<data.length;j++){ 
			...
			content+="<td>"+fnChangeName(data[j]["fireproofSignCode"],fireproofSignCode_jsarry)+"</td>"; 
			content+="<td class=\"tc\">";   
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="key_parts_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"important_parts/batch",commitData:row.value};
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
		var tourl="key_parts_xls.php?ids="+ids;
		window.open(tourl);
});

...
function processData(resultobj){
     ...
     if(success){
    	 ...
    	 for(j=0;j<data.length;j++){ 
			...
			content+="<td>"+ClearTrim(data[j]["areaNum"])+"</td>";                        
			content+="<td>"+ClearTrim(data[j]["bitNum"])+"</td>";                        
			content+="<td>"+ClearTrim(data[j]["rfidCode"])+"</td>";                        
			content+="<td class=\"tc\">";   
			content+="<a class=\"hmui-btn-primary\" href=\"patrolpoint_detail.php?id="+id+"\"></a>";
			content+="</td>"; 
			content+="</tr>"; 
...
$("#btn_add").bind("click", function(){ 
	var tourl="patrolpoint_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="patrolpoint_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 ...
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"patrol_points/batch",commitData:row.value};

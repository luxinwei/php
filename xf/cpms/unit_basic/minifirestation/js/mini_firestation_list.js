...
$("#btn_add").bind("click", function(){ 
	var tourl="mini_firestation_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="mini_firestation_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"mini_fire_stations/batch",commitData:row.value};
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
		var tourl="mini_firestation_xls.php?ids="+ids;
		window.open(tourl);
});

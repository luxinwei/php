	  ...
	  var success=resultobj.success; 
	     ...
	     if(success){
	    	 ...
	    	 for(j=0;j<data.length;j++){ 
				...
				content+="<td>"+data[j]["length"]+"</td>";             
				content+="<td class=\"tc\">";   
				content+="<a class=\"hmui-btn-primary\" href=\"tunnel_detail.php?id="+id+"\"></a>";
				content+="</td>"; 
				content+="</tr>"; 
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="tunnel_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
	    ...
	    if(!confirm("确认进行此操作吗?"))return false;
		    ...
		    var params={uri:"building_tunnels/"+row.value};

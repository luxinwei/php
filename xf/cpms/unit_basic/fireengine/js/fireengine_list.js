	...
	//	  $("#tbody_content").parent().after(JSON.stringify(resultobj));
		...
		if(success){
			...
			for(j=0;j<data.length;j++){
    			...
    			content+="<td>"+data[j]["manufacturer"]+"</td>";
				content+="<td>"+data[j]["model"]+"</td>";
				content+="<td>"+data[j]["number"]+"</td>";
				content+="<td>"+ClearTrim(data[j]["moduleName"])+"</td>";
				content+="<td>"+data[j]["description"]+"</td>";
				content+="<td class=\"tc\">";
				content+="<a class=\"hmui-btn-primary\" href=\"fireengine_detail.php?id="+id+"\"></a>";
				content+="</td>";
				content+="</tr>";
...
$("#btn_add").bind("click", function(){ 
	var tourl="fireengine_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="fireengine_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 ...
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"mainframes/batch",commitData:row.value};

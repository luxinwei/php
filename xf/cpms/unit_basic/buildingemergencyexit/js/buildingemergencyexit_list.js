	...
	////$("#pageNav").parent().after(JSON.stringify(resultobj));
     ...
     if(success){
    	 ...
    	 for(j=0;j<data.length;j++){ 
			...
			content+="<td class=\"tc\">";   
			content+="<a class=\"hmui-btn-primary\" href=\"buildingemergencyexit_detail.php?id="+id+"\"></a>";
			content+="</td>"; 
			content+="</tr>"; 
...
$("#btn_add").bind("click", function(){ 
	var tourl="buildingemergencyexit_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="buildingemergencyexit_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"building_emergency_exits/batch",commitData:row.value};
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
		var tourl="buildingemergencyexit_xls.php?ids="+ids;
		window.open(tourl);
});

	  ...
	  var success=resultobj.success; 
	     ...
	     if(success){
	    	 ...
	    	 for(j=0;j<data.length;j++){ 
				...
				content+="<td>"+fnChangeName(data[j]["buildingDrectionCode"],buildingDrectionCode_jsarry)+"</td>"; 
				content+="<td>"+data[j]["name"]+"</td>"; 
				content+="<td>"+fnChangeName(data[j]["buildingUseKindCode"],buildingUseKindCode_jsarry)+"</td>";                  
				content+="<td>"+fnChangeName(data[j]["buildingStructureCode"],buildingStructureCode_jsarry)+"</td>";              
				content+="<td>"+data[j]["height"]+"</td>";             
				content+="<td>"+data[j]["standoffDistance"]+"</td>";             
				content+="<td class=\"tc\">";   
				content+="<a class=\"hmui-btn-primary\" href=\"adjacentbuilding_detail.php?id="+id+"\"></a>";
				content+="</td>"; 
				content+="</tr>"; 
...
$("#btn_add").bind("click", function(){ 
	var tourl="adjacentbuilding_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="adjacentbuilding_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
$("#btn_del").bind("click", function(){
	  ...
	  //  if(row.row<=0){alert("请选择要删除的信息");return false;}
		...
		var url=rootPath+"/com/base/InterfaceDeleteAction.php";
		 ...
		 if(row.row>1){
		    	alert(row.value);
		    	 params={uri:"adjacent_buildings/batch",commitData:row.value};
...
$("#btn_xls").bind("click", function(){
		...
		var ids=row.value;
  		var tourl="adjacentbuilding_xls.php?ids="+ids;
		window.open(tourl);
});

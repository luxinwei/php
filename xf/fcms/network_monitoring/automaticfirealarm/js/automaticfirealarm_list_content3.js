	...
	function search(){
		...
		if(params!="")params+="&";
		params+="uri=auto_fire_alarms/electricalAlarm";
	...
	function processData(resultobj){
		...
		if(success){
			...
			for(j=0;j<data.length;j++){
				...
				content+="<td>"+data[j]["error"]+"</td>";
				content+="<td>"+ClearTrim(data[j]["state"])+"</td>";
				content+="<td class=\"tc\">";
				content+="<a class=\"hmui-btn-primary\" href=\"automaticfirealarm_list_content3_detail.php?id="+id+"&eId="+eId+"&tId="+tId+"\"></a>";
				content+="</td>";
				content+="</tr>";

...
function processData(resultobj){
	...
	if(success){
		...
		for(j=0;j<data.length;j++){
			...
			content+="<td>"+data[j]["fireLawCode"]+"</td>";
			content+="<td>"+data[j]["promulgateOffice"]+"</td>";
			content+="<td>"+data[j]["promulgateNo"]+"</td>";
			

 		 	var promulgateDate=timestampToTime(data[j]["promulgateDate"]);
 		 	promulgateDate=promulgateDate.substring(0,10);

 		 	var implementDate=timestampToTime(data[j]["implementDate"]);
 		 	implementDate=implementDate.substring(0,10);
			content+="<td>"+promulgateDate+"</td>";
			content+="<td>"+implementDate+"</td>";
				content+="<td class=\"tc\">";
			content+="<a class=\"hmui-btn-primary\" href=\"filecode_detail.php?id="+id+"\"></a>";
			content+="</td>";
			content+="</tr>";
	...
	$.post(url,params,function(responseText){
		...
		if(success){
			...
			for(j=0;j<data.length;j++){
				...
				content+="<td>"+data[j]["dataStateCode"]+"</td>";
				content+="<td>"+data[j]["content"]+"</td>";
				content+="<td class=\"tc\">";
				content+="<a class=\"hmui-btn hmui-btn-primary\" href=\"commonsenseoffire_detail.php?id="+id+"\">详情</a>";
				content+="</td>";
				content+="</tr>";

    	...
    	//$("#pageNav").parent().after(JSON.stringify(resultobj));	   
         ...
         if(success){
             ...
             for(j=0;j<data.length;j++){
         		...
         		content+="</td>"; 
     			content+="<td>"+data[j]["trainingTitle"]+"</td>";  
     			content+="<td>"+data[j]["trainingContent"]+"</td>";
     			...
     			content+="<td>"+data[j]["joiningRate"]+"</td>";                        
     			content+="<td>"+data[j]["percentRate"]+"</td>";                        
     			content+="<td class=\"tc\">";     
     			content+="<a class=\"hmui-btn-primary\" href=\"traintaskmanagement_detail.php?id="+id+"\"> </a>";
      			content+="</td>"; 
     			content+="</tr>"; 
  ...
  $("#btn_add").bind("click", function(){ 
  	var tourl="traintaskmanagement_build.php";  
  ...
  $("#btn_edit").bind("click", function(){
  	...
  	if(row.row!=1){alert("请选择一条记录");return false;}
  	var key=row.value;
  	window.location.href="traintaskmanagement_update.php?id="+key;	 	 
  });

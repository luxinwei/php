$("#btn_build").bind("click", function(){window.location.href="firefightingdevice_build.php?m="+m;});
$("[name='btn_delete']").bind("click", function(){alert("确认删除？")}); 
     ...
     if(success){
    	 ...
    	 for(j=0;j<data.length;j++){ 
			...
			content+="<td>"+data[j]["name"]+"</td>";  
 			content+="<td>"+ClearTrim(data[j]["position"])+"</td>";                        
			content+="<td>"+ClearTrim(data[j]["systemFormName"])+"</td>";                        
 			content+="<td>"+data[j]["floor"]+"</td>";                        
 			content+="<td>"+ClearTrim(data[j]["serviceStateName"])+"</td>";                        
 			content+="<td>"+data[j]["productName"]+"</td>";                                             
			content+="<td class=\"tc\">";   
			content+="<a class=\"hmui-btn-primary\" href=\"firefightingdevice_detail.php?id="+id+"\"></a>";
			content+="</td>"; 
			content+="</tr>"; 
...
$("#btn_add").bind("click", function(){ 
	var tourl="firefightingdevice_build.php";  
...
$("#btn_edit").bind("click", function(){
	...
	var key=row.value;
	window.location.href="firefightingdevice_update.php?id="+key;	 	 
});
//删除动作---删除单条数据=========================================
...
$("#btn_xls").bind("click", function(){
		...
		var tourl="firefightingdevice_xls.php?"+params;
		window.open(tourl);
});

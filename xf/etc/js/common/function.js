...
function selectData(gridObj){
	...
	activex_obj.addClass("hmui-input-select-icon");
	//$("#"+btnId).empty();
	...
	activex_obj.bind("click", function(){
	        new function showMenu() {
				...
				if(cityOffset.left>gridObj.width){
					var left_length=cityOffset.left-gridObj.width+activex_obj.outerWidth()+$("#"+btnId).outerWidth();
					$("#"+menuContentId).css({left: left_length+ "px", top:cityOffset.top + activex_obj.outerHeight() + "px"}).slideDown("fast");
				}else{
					$("#"+menuContentId).css({left:cityOffset.left + "px", top:cityOffset.top + activex_obj.outerHeight() + "px"}).slideDown("fast");
				}
	...
	var menuContent="";
	menuContent+="<div id=\""+menuContentId+"\"  style=\"display:none;position: absolute;min-width:"+gridObj.width+"px;height:"+gridObj.height+"px;background: #f0f6e4;z-index:999px;\">";
	menuContent+="</div>";
	activex_obj.after(menuContent);
...
function InitKindEditor(actives){
  ...
  document.write("<script charset=\"utf-8\"  src=\""+rootPath+"/uploadpage/kindeditor/lang/zh_CN.js\"></script>");
   $(document).ready(function(){ 
		  var uploadJson = rootPath+'/uploadpage/kindeditor/php/upload_json.php?a='+Math.random();
		  ...
		  KindEditor.ready(function(K) {
			...
			K.create('textarea[name="'+activeArray+'"]', {
						...
						fileManagerJson : rootPath+'/uploadpage/kindeditor/php/file_manager_json.php',

...
function checkLength(lsObj,lsLabel,maxLen){
	...
	if(lsValue.length>maxLen){
	     if(loMsgDiv!=null){
		  	loMsgDiv.innerHTML="<font color=red>请确保"+lsLabel+"的内容长度不超过"+maxLen+"个字符！</font>";
		 }else{
			alert("请确保"+lsLabel+"的内容长度不超过"+maxLen+"个字符！");
			getObj(lsObj).focus();
		}
...
function checkMinLength(lsObj,lsLabel,minLen){
	...
	if(lsValue.length<=minLen){
	     if(loMsgDiv!=null){
		  	loMsgDiv.innerHTML="<font color=red>请确保"+lsLabel+"的内容长度能大于"+minLen+"个字符！</font>";
		 }else{
			alert("请确保"+lsLabel+"的内容长度大于"+minLen+"个字符！");
			getObj(lsObj).focus();
		}
